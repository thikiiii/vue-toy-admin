...
const onScroll = (type: 'left' | 'right') => {
    ...
    container.scrollTo({
        left:
      type === 'left' ? container.scrollLeft - container.offsetWidth : container.scrollLeft + container.offsetWidth,
        behavior: 'smooth'
    })
}
...
// 滚动到指定位置 防抖
const scrollToActivePositionDebounce = useDebounceFn(
    () => scrollToActivePosition(route.path),
    300
)
...
<template>
  <div class="tabBar">
    <div
      v-show="scrollBtnVisible"
      class="tabBar-action-tab"
      @click="onScroll('left')"
    >
    ...
    <div ref="tabContainer" class="tabBar-tabContainer">
      ...
      >
        ...
        <icon
          v-if="!tab.meta?.affix"
          class="tabBar-tabContainer-tab-close"
          icon="mdi:close"
          size="12px"
    ...
    <context-menu v-model:visible="globalVisible" :is-right-click="false">

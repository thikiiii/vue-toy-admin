...
const isCollapsed = computed(() => mobile.isMobile ? mobile.mobileMenuVisible : sidebar.isCollapsedSidebar)
...
const collapsedHandle = (status: boolean) => {
    mobile.isMobile ? layoutStore.toggleMobileMenuVisible(status) : layoutStore.toggleCollapsed(status)
}
</script>
...
<template>
  ...
  <icon-hover-container
    content="菜单折叠"
    @click="collapsedHandle(!isCollapsed)"
  >

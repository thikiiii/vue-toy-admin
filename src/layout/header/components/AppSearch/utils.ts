...
export const searchMenus = (
    ...
    return menus.reduce<Store.MenuOption[]>((menus, menu) => {
        if (menu.meta?.title.includes(menuName)) menus.push({ ...menu, children: [] })
        
        if (menu.children?.length) {
            const matchingMenus = searchMenus(menuName, menu.children)
            if (matchingMenus.length) menus.push({ ...menu, children: matchingMenus })
        }
        return menus
    }, [])
}
...
export const handleResult = (result: Store.MenuOption[]): AppMenuSearchHistoryRecord[] => {
    ...
    const handle = (
        ...
        menus.forEach(menu => {
            ...
            if (!menu.children?.length) menuList.push({
                icon: icon as string,
